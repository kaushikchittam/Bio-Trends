...
for index in range(len(kwlist)):
  ...
  if qlist[index] != '': #or qlist[index]!= 'Q000000':
    kquery="SELECT pubID FROM records WHERE meshID=%s AND qualifID =%s;"
    cur.execute(kquery,(kwlist[index],qlist[index]))
  else:
    kquery="SELECT pubID FROM records WHERE meshID=%s;"
...
if  final_list :
  ...
  if len(final_list)!=1:
    final_query="SELECT year, COUNT(year) FROM arti where pubID in" + str(tuple(final_list))+ " GROUP BY year;"
    cur.execute(final_query)
    for row in cur:
      dict_final[row[0]]=row[1]
  else :
    final_query='SELECT year, COUNT(year) FROM arti where pubID = %s GROUP BY year;'
...
for year in years:
   ...
   query = "SELECT COUNT(year) FROM arti WHERE year = " + str(year) + ";"
   cur.execute(query)
   for row in cur:
