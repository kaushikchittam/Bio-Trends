...
$('#resetBtn').click(function(){
 ...
 $("#geneForm").validate({
                rules: {
                    keyword: "required"
                    //keywordQualifer: "required",
                    
                },
                messages: {
                    keyword: "Please select  a keyword"
	...
	//$('#checkPersist').on('change',function(){
			 //$("#timeGraphContainer").empty();
			  if($("#checkPersist").is(":checked")){
				  jsonArray.push(jsonArrayPrev[(jsonArrayPrev.length)-1]);
				  //jsonArray.push(returnData);
				  console.log(jsonArray);
					var arr=[];
					chartConfig.series=[];
					for (var i=0;i<jsonArray.length;i++){
						arr.push({name:'Query'+ (i+1),data:jsonArray[i].values});
