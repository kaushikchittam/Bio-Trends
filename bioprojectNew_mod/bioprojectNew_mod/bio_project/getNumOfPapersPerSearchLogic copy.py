...
#print tuple(union)
  ...
  if match == 'all':
    ...
    all_query="SELECT year, COUNT(year) FROM arti where pubID in" + str(tuple(intersection))+ " GROUP BY year;"
    ...
    all_query="SELECT year, COUNT(year) FROM arti where pubID in" + str(tuple(union))+ " GROUP BY year;"
    cur.execute(all_query)
    for row in cur:
      dict_final[row[0]]=row[1]
print dict_final
